...
class InstalledQueryTest(unittest.TestCase):
    ...
    def test_find_record_exact_and_case_insensitive(self):
        mods.set_installed_record("Owner-CoolMod", "1.0.0", "CoolMod", paths=["BepInEx/plugins/CoolMod/x.dll"])
        self.assertIsNotNone(mods.find_installed_record("Owner-CoolMod"), "exact-key lookup")
        self.assertIsNotNone(mods.find_installed_record("owner-coolmod"), "case-insensitive lookup")
        self.assertIsNotNone(mods.find_installed_record("OWNER-COOLMOD"))

    def test_find_record_miss_returns_none(self):
        self.assertIsNone(mods.find_installed_record("Nope-Missing"))

    def test_find_record_prefers_exact_key(self):
        # An exact-key hit must short-circuit before the case-insensitive scan.
        mods.set_installed_record("Owner-Mod", "2.3.4", "Mod", paths=["BepInEx/plugins/Mod/x.dll"])
        rec = mods.find_installed_record("Owner-Mod")
    ...
    def test_present_for_default_file_install(self):
        ...
        mods.set_installed_record("Solo-FileMod", "1.0.0", "FileMod.dll")
