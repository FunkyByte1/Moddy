...
class SetIgnoreUnusedTest(unittest.TestCase):
    def setUp(self):
        ...
        mods.set_installed_record(1, "Some.Library", version="1.0.0", filename="Some.Library")

    def test_sets_and_clears_the_flag(self):
        self.assertTrue(mods.set_ignore_unused(1, "Some.Library", True))
        self.assertTrue(mods.get_installed_record(1, "Some.Library").get("ignore_unused"))
        # Clearing drops the key entirely (kept tidy), not stores False.
        self.assertTrue(mods.set_ignore_unused(1, "Some.Library", False))
        self.assertNotIn("ignore_unused", mods.get_installed_record(1, "Some.Library"))
    ...
    def test_returns_false_for_unknown_mod(self):
        self.assertFalse(mods.set_ignore_unused(1, "Not.Installed", True))
