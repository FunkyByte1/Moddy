...
export const nexusAdapter: PagedVenueAdapter = {
  ...
  async fetchPage(game, query, page, nsfw) {
    // Adult content is a server-side filter, so the NSFW value is part of the query.
    const data = await getNexusCatalog(game.appid, query, page, nsfw);
