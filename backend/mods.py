...
async def install_mod(game: GameProfile, install_dir: str, mod: ModInfo, version: str | None = None, url: str | None = None, variant: str | None = None) -> "bool | None | dict":
    ...
    if mod.source.install_type == "zip_palworld":
        return await _install_mod_zip_palworld(game, install_dir, mods_path, mod, version, url)
...
def _pw_lua_placements(files: "list[tuple[str, str]]", mod: ModInfo) -> "list[tuple[str, str]]":
    ...
    if len(top_dirs) == 1 and not top_files and not any(d.lower() in ("scripts", "dlls") for d in top_dirs):
        ...
        modname, prefix = _safe_folder_name(root_dir), root_dir + "/"
    else:
        modname, prefix = _safe_folder_name(mod.filename), ""
...
def _palworld_commit(install_dir: str, mods_path: str, mod: ModInfo, version: str | None,
    ...
    placed: list[str] = []
    is_foreign = _overwrite_guard(install_dir, mods_path, mod, [d for _s, d in staged])
    with _StagedInstall(install_dir, is_foreign=is_foreign) as txn:
        for p in old_paths:
            txn.retire(p)
    ...
    set_installed_record(mod.id, version or "latest", mod.filename, paths=placed, mod=mod)
