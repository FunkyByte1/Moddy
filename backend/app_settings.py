...
def _load() -> dict:
    ...
    the cache was built. The README documents setting the Nexus key by editing settings.json
    directly (Desktop Mode / SSH), so a cache that never reloaded would keep serving stale
    values — a blank key, NSFW off — until the next plugin restart. Keying the cache on mtime
    picks external edits up on the next read."""
